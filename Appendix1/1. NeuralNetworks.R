...
packages <- c( "data.table","tidyverse","ggplot2","ggExtra","formatR","broom",
               "gridExtra","skimr","here","Hmisc","RColorBrewer", "reshape")#,"gmm")
...
## ----neuron, out.width = "6cm", fig.margin=TRUE, fig.cap="Depiction of a neuronal cell, with dendrites, axon, cell body, and nerve endings. (Image taken from Wikimedia Commons)", echo=F----
knitr::include_graphics(here("figures", "dendrites.pdf"))
...
covariates <- nhefs[,c("qsmk", "sex", "age", "income", "sbp", "dbp", "price71", "tax71", "race")]

nnet1 <- nnet(x=covariates, y=outcome, size=5)
...
pdf(here("figures","nnet_example.pdf"), width = 7, height = 7)
plot.nnet(nnet1, alpha.val = 0.5, circle.col = list('lightgray', 'white'), bord.col = 'black')
dev.off()


## ----nneteg, out.width = "10cm", fig.align="center", fig.cap="Structure of a simple neural net with 5 nodes fit to the NHEFS data. Black lines represent positive associations, gray lines represent negative associations. The size of each line corresponds roughly to the association magnitude. Code for this figure was obtained from: https://stackoverflow.com/questions/12447852/", echo=F----
knitr::include_graphics(here("figures", "nnet_example.pdf"))
